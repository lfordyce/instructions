...
pub fn rotate_left(nums: &mut Vec<i32>, k: i32) {
    let mut temp = Vec::with_capacity(nums.len());
    for i in 0..nums.len() {
        let rotation = (i + k as usize) % nums.len();
        temp.push(nums[rotation]);
    }
    for i in 0..nums.len() {
        nums[i] = temp[i]
    }
...
// pub fn rotate_left_in_place(nums: &mut Vec<i32>, k: i32) {
//     let mut idx = 0;
//     let mut num = nums[0];
//     let mut start_idx = 0;
//     for _ in 0..nums.len() {
//         idx = (idx + k as usize) % nums.len();
//         let temp = num;
//         num = nums[idx];
//         nums[idx] = temp;
//         if idx == start_idx {
